...
print("Reaction Advection 2-D dt: ", dt)

# 3-D case
dt = np.minimum(1 / np.max(abs(np.diag(bateman))), 1.0 / eig_3D)
print("Reaction Advection 3-D dt: ", dt)
...
print("Reaction Advection Diffusion 2-D dt: ", dt)

# 3-D case
dt = np.min(
    [1 / np.max(abs(np.diag(bateman))), 1.0 / eig_3D, 1.0 / eig_dif_3D])
print("Reaction Advection Diffusion 3-D dt: ", dt)
