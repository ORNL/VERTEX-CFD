                      ...
                      public PHX::EvaluatorDerived<EvalType, panzer::Traits>
{
    ...
    void evaluateFields(typename panzer::Traits::EvalData d) override
    {
        ...
        _lagrange_pressure.deep_copy(1.5);
        ...
        Kokkos::parallel_for(
            "incompressible lsvof free slip test dependencies",
              ...
              const LSVOFModel lsvof_model)
{
    ...
    const auto dep_eval = Teuchos::rcp(new Dependencies<EvalType>(
        *test_fixture.ir, continuity_model, lsvof_model));
    test_fixture.registerEvaluator<EvalType>(dep_eval);

    // Create phase array
    const std::vector<std::string> dof_phase_list
        = {"alpha_salt", "alpha_lime"};
    ...
    auto free_slip_eval = Teuchos::rcp(
        new BoundaryCondition::IncompressibleLSVOFFreeSlip<EvalType,
                                                           panzer::Traits,
                                                           num_space_dim>(
            *test_fixture.ir,
            num_lsvof_dofs,
    ...
    {
        test_fixture.registerTestField<EvalType>(
            free_slip_eval->_boundary_alphas);
    }
    else if (lsvof_model_name == "CLS")
    {
        test_fixture.registerTestField<EvalType>(free_slip_eval->_boundary_phi);
    }
    // Evaluate incompressible LSVOF free slip
    ...
    const double exp_vel_2d[2] = {0.671875, -0.34375};
    const double exp_vel_3d[3] = {0.4609375, 0.078125, 0.7578125};
    const double* exp_vel = num_space_dim == 3 ? exp_vel_3d : exp_vel_2d;

    const double exp_grad_vel_0_3d[3] = {1.71875, -3.4375, 5.15625};
    const double exp_grad_vel_1_3d[3] = {3.4375, -6.875, 10.3125};
    const double exp_grad_vel_2_3d[3] = {0.859375, -1.71875, 2.578125};

    const double exp_grad_vel_0_2d[2] = {0.453125, -0.90625};
    const double exp_grad_vel_1_2d[2] = {0.90625, -1.8125};
    ...
    for (int qp = 0; qp < num_point; ++qp)
    {
        ...
        for (int d = 0; d < num_space_dim; d++)
        {
            ...
            const auto boundary_grad_velocity_1_result
                = test_fixture.getTestFieldData<EvalType>(
                    free_slip_eval->_boundary_grad_velocity[1]);
            EXPECT_DOUBLE_EQ(
                exp_grad_vel_1[d],
            ...
            if (num_space_dim > 2)
            {
                const auto boundary_grad_velocity_2_result
                    = test_fixture.getTestFieldData<EvalType>(
                        free_slip_eval->_boundary_grad_velocity[2]);
                EXPECT_DOUBLE_EQ(
                    exp_grad_vel_2[d],
...
//---------------------------------------------------------------------------//
// 2-D incompressible lsvof free slip
...
//---------------------------------------------------------------------------//
// 2-D incompressible CLS free slip
...
//---------------------------------------------------------------------------//
// 3-D incompressible lsvof free slip
