#ifndef VERTEXCFD_UTILS_SMOOTHMIN_HPP
#define VERTEXCFD_UTILS_SMOOTHMIN_HPP
...
{
//---------------------------------------------------------------------------//
// Implementation of smooth "min" using Sacado expressions.
// The third template parameter specifies whether the second parameter is a
// constant (non-FAD). The "min" function is commutative, so we do not need
...
// parameter is constant) is handled within the specializations of the "min"
// function below.
//---------------------------------------------------------------------------//
...
class SmoothMinOp<T1, T2, false, Sacado::Fad::Exp::ExprSpecDefault>
    : public Expr<SmoothMinOp<T1, T2, false, Sacado::Fad::Exp::ExprSpecDefault>>
{
  public:
    ...
    SACADO_INLINE_FUNCTION explicit SmoothMinOp(const T1& x,
                                                const T2& y,
                                                double tol)
        : x_(x)
        , y_(y)
...
template<typename T1, typename T2>
class SmoothMinOp<T1, T2, true, Sacado::Fad::Exp::ExprSpecDefault>
...
SACADO_INLINE_FUNCTION
    ...
    min(const T x, const T y, const double tol)
{
    if (x >= y + tol)
    {
        return y;
...
// Specialization when both arguments are FAD expressions
...
template<typename T>
SACADO_INLINE_FUNCTION SmoothMinOp<typename Expr<T>::derived_type,
                                   typename T::value_type,
                                   true,
                                   typename T::expr_spec_type>
min(const typename T::value_type& c, const Expr<T>& y, const double tol)
{
    using ConstT = typename T::value_type;
    using expr_t = SmoothMinOp<typename Expr<T>::derived_type,
                               ConstT,
                               true,
                               typename T::expr_spec_type>;
...
SACADO_INLINE_FUNCTION SmoothMinOp<typename Expr<T>::derived_type,
                                   ...
                                   true,
                                   typename T::expr_spec_type>
min(const Expr<T>& x, const typename T::value_type& c, const double tol)
{
    using ConstT = typename T::value_type;
    using expr_t = SmoothMinOp<typename Expr<T>::derived_type,
                               ConstT,
...
SACADO_INLINE_FUNCTION typename Sacado::mpl::disable_if<
    ...
    SmoothMinOp<typename Expr<T>::derived_type,
                typename T::scalar_type,
                true,
                typename T::expr_spec_type>>::type
min(const typename T::scalar_type& c, const Expr<T>& y, const double tol)
{
...
SACADO_INLINE_FUNCTION typename Sacado::mpl::disable_if<
    ...
    SmoothMinOp<typename Expr<T>::derived_type,
                ...
                true,
                typename T::expr_spec_type>>::type
min(const Expr<T>& x, const typename T::scalar_type& c, const double tol)
{
    using ConstT = typename T::scalar_type;
    using expr_t = SmoothMinOp<typename Expr<T>::derived_type,
                               ConstT,
...
using VertexCFD::SmoothMath::SmoothMinOp;
...
struct ExprLevel<SmoothMinOp<T1, T2, c2, E>>
{
...
template<typename T1, typename T2, bool c2, typename E>
struct IsFadExpr<SmoothMinOp<T1, T2, c2, E>>
{
...
template<typename T1, typename T2, bool c2, typename E>
struct IsExpr<VertexCFD::SmoothMath::SmoothMinOp<T1, T2, c2, E>>
{
    static const unsigned value = true;
};

template<typename T1, typename T2, bool c2, typename E>
struct BaseExprType<VertexCFD::SmoothMath::SmoothMinOp<T1, T2, c2, E>>
{
...
template<typename T1, typename T2, bool c2, typename E>
struct IsSimdType<VertexCFD::SmoothMath::SmoothMinOp<T1, T2, c2, E>>
{
    static const bool value
        = IsSimdType<typename VertexCFD::SmoothMath::
                         SmoothMinOp<T1, T2, c2, E>::scalar_type>::value;
};

template<typename T1, typename T2, bool c2, typename E>
struct ValueType<VertexCFD::SmoothMath::SmoothMinOp<T1, T2, c2, E>>
{
    using type =
        typename VertexCFD::SmoothMath::SmoothMinOp<T1, T2, c2, E>::value_type;
};

template<typename T1, typename T2, bool c2, typename E>
struct ScalarType<VertexCFD::SmoothMath::SmoothMinOp<T1, T2, c2, E>>
{
    using type =
        typename VertexCFD::SmoothMath::SmoothMinOp<T1, T2, c2, E>::scalar_type;
};

} // namespace Sacado

#endif // end VERTEXCFD_UTILS_SMOOTHMIN_HPP
