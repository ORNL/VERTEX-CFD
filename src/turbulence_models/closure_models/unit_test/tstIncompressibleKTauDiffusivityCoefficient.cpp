                      ...
                      public PHX::EvaluatorDerived<EvalType, panzer::Traits>
{
        ...
        , _nu("kinematic_viscosity", ir.dl_scalar)
    {
        ...
        this->setName(
            "K-Tau Incompressible Diffusivity Coefficient Unit "
            "Test "
            "Dependencies");
    }
    ...
    void evaluateFields(typename panzer::Traits::EvalData /**d**/) override
    {
        ...
        _turb_specific_dissipation_rate.deep_copy(_tau);
        _nu.deep_copy(0.25);
    }
};
...
void testEval()
{
    ...
    // Eval dependencies
    const auto deps
        = Teuchos::rcp(new Dependencies<EvalType>(ir, k_value, t_value));
    ...
    const auto fv_var_t
        = test_fixture.getTestFieldData<EvalType>(eval->_diffusivity_var_t);

    // Expected values
    const int num_point = ir.num_points;
    const double exp_diffusivity_var_k = nu + sigma_k * k_value * t_value;
    const double exp_diffusivity_var_t = nu + sigma_t * k_value * t_value;

    // Assert values
    for (int qp = 0; qp < num_point; ++qp)
    {
        EXPECT_DOUBLE_EQ(exp_diffusivity_var_k, fieldValue(fv_var_k, 0, qp));
        EXPECT_DOUBLE_EQ(exp_diffusivity_var_t, fieldValue(fv_var_t, 0, qp));
    }
}

//-----------------------------------------------------------------//
TEST(IncompressibleKTauDiffusivityCoefficient, Residual)
{
    testEval<panzer::Traits::Residual>();
}

//-----------------------------------------------------------------//
TEST(IncompressibleKTauDiffusivityCoefficient, Jacobian)
{
