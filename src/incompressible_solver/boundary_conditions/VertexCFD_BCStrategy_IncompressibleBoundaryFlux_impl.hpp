    ...
    const Teuchos::ParameterList& /**user_data**/)
{
    ...
    this->_bc_params = *(this->m_bc.params());
    this->getModelID(
        this->_bc_params, side_pb, this->_model_id, this->_side_pb_list);

    // Temperature equation boolean
    const std::string continuity_model
        = this->_side_pb_list.template isType<std::string>("Continuity Model")
              ? this->_side_pb_list.template get<std::string>(
                    "Continuity Model")
              : "AC";
    ...
    _build_viscous_flux
        = this->_side_pb_list.template get<bool>("Build Viscous Flux");

    // Temperature equation boolean
    _build_temp_equ = this->_side_pb_list.template isType<bool>(
                          "Build Temperature Equation")
                          ? this->_side_pb_list.template get<bool>(
                                "Build Temperature Equation")
                          : false;

    // Electric potential equation boolean
    _build_ind_less_equ = this->_side_pb_list.template isType<bool>(
    ...
    const std::string turbulence_model_name
        = this->_side_pb_list.template isType<std::string>(
              "Turbulence Model Name")
              ? this->_side_pb_list.template get<std::string>(
                    "Turbulence Model Name")
              : "No Turbulence Model";
    _turbulence_model_bool
        = turbulence_model_name == "No Turbulence Model" ? false : true;
    ...
    const Teuchos::ParameterList& user_data) const
{
    ...
    if (closure_models.sublist(this->_model_id).isSublist("Stability Parameters"))
    {
        stability_param_list = closure_models.sublist(this->_model_id)
                                   .sublist("Stability Parameters");
    }

    // Fluid properties
    Teuchos::ParameterList fluid_prop_list
        = closure_models.sublist(this->_model_id).sublist("Fluid Properties");
    ...
    if (_turbulence_model_bool)
    {
        ...
        turb_params = closure_models.sublist(this->_model_id)
                          .sublist("Turbulence Parameters");
    }
    turb_params.set("Use Turbulence Model", _turbulence_model_bool);

    // Create boundary state operators for NS equations and EP equation
    // NS equations
    const auto ns_bc_sublist = this->_bc_params.isSublist("Navier-Stokes")
                                   ? this->_bc_params.sublist("Navier-Stokes")
                                   : this->_bc_params;
    auto incomp_ns_boundary_state_op = IncompressibleBoundaryStateFactory<
        EvalType,
        panzer::Traits,
        num_space_dim>::create(*ir, ns_bc_sublist, fluid_prop_list);
    this->template registerEvaluator<EvalType>(fm, incomp_ns_boundary_state_op);

    // EP equations
    if (_build_ind_less_equ)
    {
        const auto ep_bc_sublist
            = this->_bc_params.sublist("Electric Potential");
    ...
    auto convective_flux_op = Teuchos::rcp(
        new ClosureModel::IncompressibleConvectiveFlux<EvalType,
                                                       panzer::Traits,
                                                       num_space_dim>(
            *ir, fluid_prop_list, "BOUNDARY_", "BOUNDARY_"));
    ...
    if (_build_viscous_flux)
    {
        ...
        const Teuchos::ParameterList ns_bc_params
            = this->_bc_params.isSublist("Navier-Stokes")
                  ? this->_bc_params.sublist("Navier-Stokes")
                  : this->_bc_params;
        ...
        // Register penalty and viscous gradient operators for each NS
        // equation.
        for (auto& pair : _equ_dof_ns_pair)
        {
            this->registerPenaltyAndViscousGradientOperator(
                pair, fm, side_pb, this->_bc_params);
        }
    ...
    if (_turbulence_model_bool)
    {
        ...
        const auto turb_bc_params
            = _equ_dof_tm_pair.empty()
                  ? Teuchos::ParameterList()
                  : this->_bc_params.sublist("Turbulence Model");
        ...
        const auto tm_boundary_state_op = TurbulenceBoundaryStateFactory<
            EvalType,
            panzer::Traits,
            num_space_dim>::create(*ir, turb_bc_params, user_data, turb_params);
        ...
        for (auto& pair_tm : _equ_dof_tm_pair)
        {
            ...
            BoundaryFluxBase<EvalType, NumSpaceDim>::
                registerPenaltyAndViscousGradientOperator(
                    pair_tm, fm, side_pb, this->_bc_params);
