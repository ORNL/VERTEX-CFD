                      ...
                      public PHX::EvaluatorDerived<EvalType, panzer::Traits>
{
    ...
    Dependencies(const panzer::IntegrationRule& ir)
        ...
        , exact_ep("Exact_electric_potential", ir.dl_scalar)
        , phi("lagrange_pressure", ir.dl_scalar)
        , ep("electric_potential", ir.dl_scalar)
    {
        // exact solution
        ...
        this->setName(
            "Solid Electric Potential Error Norms Unit Test Dependencies");
    }
...
void testEval()
{
    ...
    const auto eval = Teuchos::rcp(
        new ClosureModel::SolidElectricPotentialErrorNorms<EvalType,
                                                           panzer::Traits,
                                                           num_space_dim>(ir));
    ...
    const auto L1_error_ep_equ = test_fixture.getTestFieldData<EvalType>(
        eval->_L1_error_electric_potential_equation);
    const auto L2_error_ep_equ = test_fixture.getTestFieldData<EvalType>(
        eval->_L2_error_electric_potential_equation);
...
//---------------------------------------------------------------------------//
TEST(SolidElectricPotentialError, Residual)
{
    testEval<panzer::Traits::Residual, 3>();
}

//---------------------------------------------------------------------------//
TEST(SolidElectricPotentialError, Jacobian)
{
