#ifndef VERTEXCFD_SOLIDELECTRICBOUNDARYSTATE_FACTORY_HPP
#define VERTEXCFD_SOLIDELECTRICBOUNDARYSTATE_FACTORY_HPP
...
class SolidElectricBoundaryStateFactory
{
  public:
           ...
           const Teuchos::ParameterList& closure_model)
    {
        ...
        // Register electric conductivity to be used on the boundary based on
        ...
        if (closure_type == "SolidElectricConductivity")
        {
            const auto mat_prop = Teuchos::rcp(
                new ClosureModel::SolidElectricConductivity<EvalType,
                                                            panzer::Traits>(
                    ir, mat_prop_list));
        ...
        if (bc_params.isType<std::string>("Type"))
        {
            ...
            {
                state = Teuchos::rcp(new TurbulenceSymmetry<EvalType, Traits>(
                    ir, "electric_potential"));
                evaluators.push_back(state);
                found_model = true;
            }
            ...
            if (!found_model)
            {
                ...
                msg += "for the solid induction-less MHD equation are:\n";
                msg += "Extrapolate,\n";
                msg += "Fixed,\n";
                msg += "InsulatingWall,\n";
                msg += "\n";
...
#endif // end VERTEXCFD_SOLIDELECTRICBOUNDARYSTATE_FACTORY_HPP
